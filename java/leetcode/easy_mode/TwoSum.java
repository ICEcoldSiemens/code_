...
public class TwoSum {
    ...
    static void attemptTwoTwoSum(int[] listofNums, int target){
        Map<Integer, Integer> graph = new HashMap<>();

        for(int i = 0; i < listofNums.length; i++){
            int result = target - listofNums[i];
            if(graph.containsKey(result)){
                System.out.println("Solution: " + Arrays.toString(new int[]{graph.get(result), i}));
            } else {
                graph.put(listofNums[i], i);
            }
        }
    }
    ...
    public static void main(String[] args){
         ...
         //attemptOneTwoSum(nums,target); // 49 milliseconds with a complexity of O(n^2)
           attemptTwoTwoSum(nums, target); // 3 milliseconds with a complexity of O(n)
    }
}
